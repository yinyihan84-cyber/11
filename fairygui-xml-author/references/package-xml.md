...
图片是否九宫格、平铺、禁用平滑，在 `package.xml` 的 `<image>` 资源声明阶段确定。任务来自效果图或截图时，必须回看效果图里的视觉角色和组件显示尺寸来判断资源语义。
...
`package.xml` 不写布局，但它必须承接效果图推导出的图片资源语义。写 `<image>` 声明前，先比对效果图中的用途、切图原始尺寸和组件 XML 中的显示尺寸。

- 效果图中承担面板底、按钮底、卡片底、标签底、进度槽、边框、圆角底、描边或阴影底的图片，如果显示尺寸与原图尺寸不一致，优先补 `scale="9grid"` 和 `scale9grid`。
- 纯图标、头像、插画、照片、文字图、徽章主体、不可拆分装饰不因尺寸变化自动写九宫格。
- 连续纹理、条纹、格子、噪声背景优先判断 `scale="tile"`，不要误判为九宫格。
- 能从效果图判断“需要九宫格”，但缺少可验证切线时，标记 `scale9grid` 待确认；不要编造数值。
- 已有项目样本或资源面板给出九宫格时，以样本为准；效果图只用于发现漏声明或明显不合理的资源语义。
